...
> 生成日期：2026-06-23  
> 执行者：Codex  
> 适用范围：个人多端同步记账软件 Web 前端 Vue 工程  
> 参考文档：`docs/PRD.md`、`docs/ARCH.md`、`docs/API.md`、`docs/DB.md`、`docs/前端开发企业规范 - Vue 版.docx`

## 1. 目标

本规范用于把 Vue 企业开发规范落到实际工程结构、接口调用、类型定义、状态管理和联调流程中，确保前端工程具备以下特征：

- 分类清晰：页面、接口、业务编排、状态、工具函数各有边界。
- 后端调用清晰：所有 REST API 与 `docs/API.md` 的接口分组一一对应。
- 类型明确：请求参数、响应数据、分页结构、错误结构均有 TypeScript 类型。
- 可维护：页面只负责展示和交互，业务流程沉到 composables/services/api。
...
该技术栈与 `docs/ARCH.md` 当前 Web 端架构保持一致。未经确认，不允许自行替换框架、UI 组件库、路由、状态管理和请求库。
...
│   ├── assets/               # 静态资源
│   ├── components/           # 跨业务通用组件
│   ├── constants/            # 枚举、字典、路由常量
│   ├── composables/          # 跨页面复用组合式函数
│   ├── layouts/              # 主布局、登录布局
│   ├── router/               # 路由配置、鉴权守卫
...
| `views` | 组织页面布局、表单、表格和交互 | 禁止直接写 Axios |
| `composables` | 封装页面状态、查询、提交、表单逻辑 | 禁止沉淀全局状态 |
| `stores` | 登录态、用户信息、主题、字典缓存等全局状态 | 禁止存放普通页面弹窗和表单状态 |
| `utils` | 无副作用工具函数 | 禁止依赖组件实例状态 |
...
- `composables`：负责页面查询、提交、Element Plus Form、局部状态。
- `views/components`：负责展示、交互和组件组合。

简单 CRUD 页面可以由 composables 直接调用 `api/modules`，但只要出现以下情况，必须增加 `services`：
...
组合式函数命名使用 `useXxx`，按职责拆分：

```text
views/transactions/composables/
...
- `useTransactionList`：管理筛选条件、分页、列表 loading、列表查询。
- `useTransactionForm`：管理表单初始值、校验、编辑回显、重置。
- `useTransactionActions`：管理新增、更新、删除、启停等动作。

组合式函数边界：

- 可以调用 `api/modules` 或 `services`。
- 可以持有页面局部状态。
- 不直接操作路由守卫。
- 不持久化全局状态。
- 不把页面状态塞进 Pinia。
...
- 大型图表、复杂弹窗、低频页面使用异步组件。
- 表格列定义、枚举 map、默认筛选值应使用稳定常量。
- 独立接口并行加载时使用 `Promise.all`，避免无意义瀑布请求。
- 大列表优先使用服务端分页，不一次性加载全量数据。

## 9. Pinia 状态规范

Pinia 只存放全局状态和跨页面共享状态。
...
├── index.vue
├── components/
│   ├── TransactionSearchForm.vue
│   ├── TransactionTable.vue
│   └── TransactionFormDrawer.vue
├── composables/
│   ├── useTransactionList.ts
│   └── useTransactionForm.ts
...
- `index.vue`：组合页面，不堆业务细节。
- `components`：只接收 props 和 emits，不直接调用后端接口。
- `composables`：封装查询、提交、删除、弹窗、表单回显。
